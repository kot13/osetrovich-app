...
- **Push (API 0.10.0)** — после входа и при обновлении FCM-токена клиент регистрирует устройство через `PUT /profile/push-token`; in-app уведомления и бейдж загружаются с `GET /notifications` и `GET /notifications/unread-count`
